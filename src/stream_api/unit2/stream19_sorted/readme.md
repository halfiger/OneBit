...
Ось кілька схожих і простіших завдань для тренування сортування в Java Stream API. Почнемо з м’яких варіантів — так, щоб рука й око набилися.
У кожної задачі є умова, вхід, очікуваний результат, підказка та початковий шаблон коду.
...
-
    ...
    public void practice5 () {
        List<Integer> nums = List.of(3, 2, 5, 6, 1, 4);
        nums.stream1().sorted(Comparator.comparingInt(n->(int)n%2).thenComparing(n->(int)n)).collect(Collectors.toList());
    }
...
.thenComparing(Comparator.naturalOrder())
    ...
    "banana", "cherry", "kiwi", "grape");
        return words.stream1().sorted(Comparator
